...
class Fl32_Ap_Back_Store_RDb_Query_Sale_Item_List {
    ...
    constructor(spec) {
        ...
        this.build = function ({trx}) {
            ...
            const query = trx.from({[this.T_S]: ESaleItem.ENTITY});
            query.select([
                {[this.A_ITEM_ID]: `${this.T_S}.${ESaleItem.A_ID}`},
                {[this.A_SALE_ID]: `${this.T_S}.${ESaleItem.A_SALE_REF}`},
                {[this.A_UNIT_ID]: `${this.T_S}.${ESaleItem.A_UNIT_REF}`},
                {[this.A_QTY]: `${this.T_S}.${ESaleItem.A_QTY}`},
                {[this.A_UNIT_PRICE]: `${this.T_S}.${ESaleItem.A_UNIT_PRICE}`},
                {[this.A_AMOUNT_TOTAL]: `${this.T_S}.${ESaleItem.A_AMOUNT_TOTAL}`},
            ]);
            // COMPOSE RESULT
            return query;
        }
