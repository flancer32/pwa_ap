/**
 * Query to get list of products units attributes.
 *
 * @namespace Fl32_Ap_Back_Store_RDb_Query_Product_Unit_Attr_List
 */
// MODULE'S VARS
const NS = 'Fl32_Ap_Back_Store_RDb_Query_Product_Unit_Attr_List';

/**
 * Factory to create builder to get queries.
 *
 * @memberOf Fl32_Ap_Back_Store_RDb_Query_Product_Unit_Attr_List
 * @returns {function(*): *}
 */
function Factory(spec) {
    ...
    function queryBuilder({trx, lang}) {
        ...
        const query = trx.from({[me.T_U]: EUnit.ENTITY});
        query.select([
            {[me.A_UNIT_ID]: `${me.T_U}.${EUnit.A_ID}`},
            {[me.A_CARD_ID]: `${me.T_U}.${EUnit.A_CARD_REF}`},
            {[me.A_SKU]: `${me.T_U}.${EUnit.A_SKU}`},
        ]);
        // left join product_unit_attr_value
        query.leftOuterJoin(
            {[me.T_UV]: EUnitAV.ENTITY},
            `${me.T_UV}.${EUnitAV.A_UNIT_REF}`,
            `${me.T_U}.${EUnit.A_ID}`);
        query.select([{[me.A_VALUE_ID]: `${me.T_UV}.${EUnitAV.A_VALUE_REF}`}]);
        // left join attr_value
        query.leftOuterJoin(
            {[me.T_AV]: EAttrVal.ENTITY},
            `${me.T_AV}.${EAttrVal.A_ID}`,
            `${me.T_UV}.${EUnitAV.A_VALUE_REF}`);
    ...
    queryBuilder.A_SKU = 'sku';
    queryBuilder.A_UNIT_ID = 'unitId';
